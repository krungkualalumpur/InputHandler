...
export type InputHandler = {
	...
	GetInputStates : (InputHandler) -> {
		[number] : Enum.KeyCode | Enum.UserInputType
	},
	GetActiveInputMaps : (InputHandler) -> {
		[number] : Map & {InputName : string}
	},
	Destroy : (InputHandler) -> nil,
	
	getClientPlatform : () -> PlatformType
}
--constants
--references
--- local functions
...
function inputHandler:GetInputStates()
	return table.clone(self._InputState)
end

function inputHandler:GetActiveInputMaps()
	local activeMaps : {Map & {InputName : string}} = {}
	for inputName,v in pairs(self._InputMap) do
		if v.isActive then
			local activeMap : Map & {InputName : string} = table.clone(v) :: any
			activeMap.InputName =  inputName
			table.insert(activeMaps, activeMap)
		end
	end
	return activeMaps
end

inputHandler.getClientPlatform = getClientPlatform
